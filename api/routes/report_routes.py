...
async def show_login(request: Request):
    return request.app.templates.TemplateResponse(request, "login.html", {"request": request})
...
async def show_signup(request: Request):
    return request.app.templates.TemplateResponse(request, "signup.html", {"request": request})
...
async def signup(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if len(password.encode("utf-8")) > 72:
        return request.app.templates.TemplateResponse(
            request,
            "signup.html",
            {"request": request, "error": "Password cannot exceed 72 bytes"},
        )
...
async def dashboard(request: Request):
    ...
    return request.app.templates.TemplateResponse(request, "dashboard.html", {"request": request, "user": SESSIONS[session_id]})
